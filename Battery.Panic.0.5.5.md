# Battery Panic 0.5.5 Release Notes

Battery Panic 0.5.5 fixes a startup visibility issue that could make Battery Panic appear to do nothing after launch.

## What's New

- More reliable menu bar battery display:
  - Battery Panic now uses a native macOS status symbol plus percent text.
  - The menu bar item is variable-width, so the percent text is less likely to be clipped or hidden.
  - A visible `--%` placeholder appears immediately while the real battery value is loading.
