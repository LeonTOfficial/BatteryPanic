...
private func drawSettingsPreview() -> NSImage {
    ...
    drawText("Battery Panic", x: 340, y: 600, size: 34, weight: .bold, color: .white)
    drawText("Settings", x: 342, y: 570, size: 18, weight: .medium, color: muted)
    section(title: "Battery threshold", body: "Warn below 10%", y: 440)
    section(title: "Pulsing overlay", body: "Speed 1.0x  /  Intensity 100%  /  Live preview", y: 345)
    section(title: "Audio warning alerts", body: "Repeating siren  /  Test Sound", y: 250)
    drawText("Created by Leon.T  -  GitHub: LeonTOfficial", x: 340, y: 165, size: 16, weight: .medium, color: .linkColor)
    image.unlockFocus()
    return image
}
