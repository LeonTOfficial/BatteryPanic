# Battery Panic 0.5.6 Release Notes

Battery Panic 0.5.6 focuses on reliability: the menu bar battery status is now shown as plain text, and widget updates are no longer part of the normal app startup path.

## What's New

- More reliable menu bar battery display:
  - Battery Panic now shows a direct text label such as `Battery 80%`, `Charging 100%`, or `Low 9%`.
  - The display no longer depends on a tinted system icon that could appear hidden or clipped.
  - A visible `Battery --%` placeholder appears immediately while the real battery value is loading.
