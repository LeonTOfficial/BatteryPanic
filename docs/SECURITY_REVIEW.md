...
Battery Panic 0.6.0 is suitable for public source and release packaging under the documented distribution limits after local verification.

## Reviewed Areas

- Battery status access through IOKit power APIs.
- Local seven-day battery history retention and persistence.
...
- Battery history contains only timestamps, percentages, and power-state transitions, stays in local Application Support, and is never uploaded.
...
- The current public binary remains ad-hoc signed, Apple silicon only, and not notarized; users may need to approve the first launch in Privacy & Security.
...
ditto -x -k "outputs/Battery.Panic.0.6.0.zip" /tmp/BatteryPanicZipCheck
